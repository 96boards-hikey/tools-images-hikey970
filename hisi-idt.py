...
class bootdownload(object):
    ...
    startframe = {
        'hi3660':[0xFE,0x00,0xFF,0x01,0x00,0x00,0x00,0x04,0x00,0x00,0x02,0x01]
    }
    ...
    bootdownloadaddress0 = {
        #'hi3716cv200':0xF9800800
	'hi3660':0x00022000
    }

    bootdownloadaddress1 = {
        #'hi3716cv200':0x07000000
	'hi3660':0x60049000
    }

    bootdownloadaddress2 = {
        #'hi3716cv200':0x07000000
	'hi3660':0x16800000
    }

    bootdownloadaddress3 = {
	'hi3660':0x10000000
    }
    ...
    def sendheadframe(self,length,address):
        ...
        data = bytearray(self.headframe[self.chip])
        crc = self.calc_crc(data)
    ...
    def senddataframe(self,seq,data):
        self.s.timeout = 0.15
        head = bytearray(b'\xDA')
        head.append(seq&0xFF)
        head.append((~seq)&0xFF)
        data = head + data
    ...
    def sendtailframe(self,seq):
        data = bytearray(b'\xED')
        data.append(seq&0xFF)
        data.append((~seq)&0xFF)
        crc = self.calc_crc(data)
        data.append((crc >> 8)&0xff)
        data.append(crc&0xff)
    ...
    def download(self, filename1, filename2, filename3, filename4):
        ...
        self.senddata(data,self.bootdownloadaddress0[self.chip])
        print('Done\n')
        ...
        if filename2:
            ...
            self.senddata(data,self.bootdownloadaddress1[self.chip])
            print('Done\n')
        ...
        if filename3:
            ...
            print('Sending', filename3, '...')
            self.senddata(data,self.bootdownloadaddress2[self.chip])
            print('Done\n')
        ...
        if filename4:
            ...
            print('Sending', filename4, '...')
            self.senddata(data,self.bootdownloadaddress3[self.chip])
            print('Done\n')



def burnboot(chiptype, serialport, filename1, filename2='', filename3='', filename4=''):
    downloader = bootdownload(chiptype, serialport)
    downloader.download(filename1, filename2, filename3, filename4)
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"hd:",["img1=","img2=","img3=","img4="])
    ...
    if (img3):
        if not os.path.isfile(img3):
            print("Image don't exists:", img3)
            sys.exit(1)

    if (img4):
        if not os.path.isfile(img4):
            print("Image don't exists:", img4)
            sys.exit(1)

    burnboot('hi3660', dev, img1, img2, img3, img4)
